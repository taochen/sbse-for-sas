...
import org.soa.wsdream.svsm.CombineProposition;
...
public class WSSimulator {
	...
	private static final String setting = "100AS" +
			"/";
	public static int MOEA = 2; //0= moead 1=nsgaii 2=ibea
	...
	static String[] remove_strings = new String[]{
		...
		"sas-rubis_software-Availability",
		"sas-rubis_software-Compliance",
		"sas-rubis_software-Practices"
		};
	...
	public static void setup() {
		File source = new File("/Users/tao/research/monitor/ws-soa/sas");
		if (source.exists()) {
		...
		WSSOADelegate qos3 = new WSSOADelegate(3, workflow);
		WSSOADelegate qos4 = new WSSOADelegate(4, workflow);
		...
		for (Objective ob : obj) {
			if ("sas-rubis_software-Availability".equals(ob.getName())) {
				o.add(ob);
			}
		}
		
		for (Objective ob : obj) {
			if ("sas-rubis_software-Compliance".equals(ob.getName())) {
				o.add(ob);
			}
		}
		
		for (Objective ob : obj) {
			if ("sas-rubis_software-Practices".equals(ob.getName())) {
				o.add(ob);
			}
		}
		...
		for (Objective ob : o) {
			...
			} else if (qos.getName().equals("sas-rubis_software-Availability")) {
				qos.setDelegate(qos2);
			} else if (qos.getName().equals("sas-rubis_software-Compliance")) {
				qos.setDelegate(qos3);
			} else {
				qos.setDelegate(qos4);
			}
