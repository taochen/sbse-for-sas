...
public class WSWorkflow {
	...
	private static final String prefix = "/Users/"+System.getProperty("user.name")+"/research/experiments-data/public-data/services/qws/";
	...
	private static List<WSConcreteService> generateOptionalCS(){
		...
		String[] s = new String[]{"latency","throughput","availability","compliance","practices"};
		...
		for (int i = 0; i < d[0].length;i++) {
			double[] temp = new double[]{d[0][i],d[1][i],d[2][i],d[3][i],d[4][i]};
			System.out.print(temp[0]+":"+temp[1]+":"+temp[2]+":"+temp[3]+":"+temp[4]+"\n");
			//if(temp[2]==0)
			//System.out.print(temp[2]+"\n");
			list.add(new WSConcreteService(temp));
		}
	...
	private static double[] getValue(String name, int no) {
		double v = 0.0;
		int r = numbers_e[no];
		int k = 0;
		double[] array = new double[r];
		try {
			BufferedReader reader = new BufferedReader(new FileReader(prefix+name+".txt"));
			String line = null;
			...
			while ((line = reader.readLine()) != null) {
				...
				if(i % (no+1) == 0) {
					if("availability".equals(name) || "compliance".equals(name) || 
							"practices".equals(name)) {
						array[k]=Double.parseDouble(line)*0.01;
					} else {
					    array[k]=Double.parseDouble(line);
					}
					k++;
				}
